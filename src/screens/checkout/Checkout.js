...
class Checkout extends Component {
  ...
  updateSummary = () => {
    this.setState({
      ...
      billedItems: this.props.history.location.state.chcartItems,
      totalBillAmount: this.props.history.location.state.totalCartItemsValue,
    });
  };
  ...
  placeOrderClickHandler = () => {
    ...
    let itemQuantities = this.state.billedItems.map(
        function (i) {
            return {
                'item_id': i.id,
    ...
    requestBody.coupon_id = '2ddf6284-ecd0-11e8-8eb2-f2801f1b9fd1';
    requestBody.discount = 0;
    requestBody.item_quantities = itemQuantities;
    requestBody.payment_id = this.state.selectedPaymentOptionId;
    requestBody.restaurant_id = this.props.history.location.state.restaurantId;

    xhrPlaceOrderMethod.send(JSON.stringify(requestBody));
    console.log(this.props.history.location.state.restaurantId);
